...
type cache struct {
	...
	segs [segmentCount]segment // map[Key]*entry
}

func (c *cache) getSegment(h uint64) *segment {
	return c.segment(h)
}
...
func (c *cache) deleteKey(k Key) {
	seg := c.segment(sum(k))
	_, ok := seg.LoadAndDelete(k)
	if ok {
		atomic.AddInt64(&c.size, -1)
	}
}
...
func (c *cache) segment(h uint64) *segment {
